...
void ClapTrap::printMessage(std::string message, std::string flag, std::string target) {
	if (message.empty() && flag == "ATTACK") {
		std::cout << "ClapTrap " << this->getName() << " attacks " << ((!target.empty()) ? target : "NULL") << ", causing " << this->getDamage() << " points of damage" << std::endl;
	}
		...
		std::cout << "ClapTrap " << this->getName() << " has taken damage, and now has " << this->getHealth() << " points of health" << std::endl;
	}
	else {
		std::cout << "ClapTrap " << this->getName() << (!(message.empty()) ? message : "") << std::endl;
	}
}
